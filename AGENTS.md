...
本项目是学习型实现。后续每个阶段新增或修改 `minicode_lite/` 下的运行时代码时，必须同步补充简体中文教学注释。

- 核心模块和核心函数：对每条有逻辑意义的语句说明职责、数据流、状态变化、分支原因或安全边界；不必为纯导入、空行和重复语法含义添加冗余注释。
- 非核心模块和函数：至少说明模块职责、函数输入输出、关键分支、错误处理与边界；简单包入口可以保持简短。
- 注释应优先解释“为什么这样做”，必须与代码实际行为一致，不得复制陈旧或误导性说明。
- 测试文件不要求逐行注释，测试名称和断言应保持清晰即可。
...
每个阶段的学习总结必须包含“面试高频问题与参考答案”章节。没有题库或只有问题没有答案时，不得认为阶段学习总结完整。

- 每个阶段至少编写 8 道高质量问题，并为每题提供可用于口述练习的简体中文参考答案。
- 问题必须紧扣本阶段实际实现，不得生成与当前代码能力不符的空泛八股题。
- 题目应尽量覆盖：基础概念、核心对象职责、关键数据流、设计取舍、异常与安全边界、测试策略，以及与真实 MiniCode-Python 的差异。
- 参考答案先直接回答结论，再解释原因、工作原理和必要的边界；不能只给关键词或一句定义。
- 至少包含一道场景题或故障排查题，检验能否把知识用于实际 harness 问题。
- 至少包含一道取舍题，检验能否解释“为什么当前阶段这样实现，而不是一次复制完整 MiniCode”。
- 面试题既用于检验学习效果，也用于复习核心知识，因此应避免多题重复考察同一个知识点。
- 后续每完成一个新阶段，必须在测试、提交和打标签之前，把该阶段题库和答案写入对应的 `stage-summaries` 文档。
