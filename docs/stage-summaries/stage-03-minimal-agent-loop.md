...
### 1. 最小 agent loop 的核心闭环是什么？

**参考答案：** 核心闭环是 `messages -> model.next -> AgentStep`。如果得到 final assistant，就写入历史并结束；如果得到 tool calls，就先记录工具意图，执行工具，把 `ToolResult` 写成观察消息，再把更新后的历史交给模型。循环的价值是把模型推理与外部行动连接起来，而不是简单重复调用模型。

### 2. 为什么工具调用和工具结果都必须进入消息历史？

**参考答案：** 工具调用记录模型“想做什么”，工具结果记录环境“实际发生了什么”，二者通过调用 ID 配对。缺少调用意图，结果来源不清楚，provider 协议也可能不完整；缺少结果，模型下一步无法基于真实观察继续推理。完整历史还为后续 session replay 和故障诊断提供依据。

### 3. 为什么同一批 tool calls 要先全部记录，再开始写入 tool results？

**参考答案：** 一个模型步骤可能同时返回多个工具调用，它们在 provider 语义上属于同一个 assistant `tool_calls` 消息。如果记录一个调用后立刻插入结果，再记录第二个调用，就会破坏批次连续性，adapter 难以还原原始协议。先记录整批意图，再逐个执行和记录结果，既保留批次语义，也允许工具顺序执行。

### 4. `max_steps` 为什么是 agent loop 必需的安全边界？

**参考答案：** 模型可能持续调用工具、反复纠错或永远不给 final answer，没有步数预算就可能造成无限循环、额度浪费和不可控副作用。`max_steps` 把一次 turn 的资源消耗限制为可预测上限，并在耗尽时写出明确停止原因。生产系统还可能增加时间、token 和成本预算，但最小循环先用步数就能建立硬边界。

### 5. 为什么空 assistant 响应可以重试一次，但不能无限重试？

**参考答案：** 一次空响应可能来自模型格式波动，追加明确提示后重试能提高鲁棒性；连续空响应则说明 adapter、provider 或模型行为存在问题，继续重试只会消耗预算。有限重试体现“允许恢复，但必须有上限”，并且停止原因需要进入历史供调用方和测试观察。

### 6. 回调为什么属于可选展示钩子，而不应承载核心状态？

**参考答案：** `on_tool_start`、`on_tool_result` 和 `on_assistant_message` 让 CLI/TUI 展示进度，但权威状态仍是返回的消息历史。这样没有 UI 的 headless 测试也能运行，回调失败或缺失不会改变循环协议。若把状态只放在回调里，持久化和重放会依赖某个具体展示层。

### 7. 场景题：模型按脚本只应调用一次工具，但测试发现 `model.next()` 被调用三次，如何排查？

**参考答案：** 先检查第一步是否返回 tool calls，因为正常闭环至少还需要一次模型调用生成 final；再检查是否出现空 assistant 触发重试；最后检查工具结果是否被正确写回，模型是否因错误结果继续修正。应同时断言调用次数和每次收到的消息尾部，而不是只看最终文本，这样能定位多出来的控制分支。

### 8. 为什么阶段 3 暂时不引入完整 `turn_kernel`？

**参考答案：** 当前只需要证明模型、工具和最终回答之间的最短循环。真实 `turn_kernel` 还会管理 phase、verification、widening、预算和策略，如果过早引入，会让基础消息流难以观察。先把最小 loop 测透，后续才能知道哪些复杂控制逻辑值得抽离，而不是为了架构外观先制造抽象。
