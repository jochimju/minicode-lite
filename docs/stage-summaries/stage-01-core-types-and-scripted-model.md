...
### 1. 为什么 agent harness 要先定义统一核心类型，而不是直接使用某家模型 API 的 JSON？

**参考答案：** 统一类型把 provider 协议隔离在 adapter 边界外。agent loop 只需要理解 `ChatMessage`、`ToolCall` 和 `AgentStep`，不必知道 Qwen 或 OpenAI 的字段细节；测试也能直接构造领域对象而不依赖网络响应。这样更换模型供应商时主要修改 adapter，而不是重写循环、工具和会话层。

### 2. `ChatMessage`、`ToolCall` 和 `AgentStep` 分别表达什么？

**参考答案：** `ChatMessage` 表达进入上下文的历史消息；`ToolCall` 表达模型提出的一次结构化工具意图，包括调用 ID、工具名和输入；`AgentStep` 表达模型本次决策结果，要么是最终 assistant 文本，要么是一批工具调用。三者分别对应“已发生的历史”“准备执行的动作”和“模型当前输出”，不能混成一个无约束字典。

### 3. 为什么 `AgentStep` 要区分 assistant 和 tool_calls 两种分支？

**参考答案：** 两种分支对控制流的意义不同：assistant final 表示本轮可以结束，tool_calls 表示 harness 必须执行动作、记录观察并再次调用模型。如果只返回一段文本，循环就只能靠字符串解析猜测模型意图；显式分支能让类型、测试和停止条件更清楚，也方便以后增加 progress 等步骤类型。

### 4. `ScriptedModel` 和普通 `MockModelAdapter` 有什么区别？

**参考答案：** `ScriptedModel` 按预先给定的步骤序列返回结果，适合精确驱动多步流程和异常分支；普通 mock adapter 通常根据简单输入规则返回固定回答或工具调用，更接近演示替身。前者强调“可编排、可预测的测试剧本”，后者强调“没有真实 provider 时仍能跑通入口”。二者都不用于衡量模型智能。

### 5. 为什么测试 agent 基础设施时不应默认调用真实大模型？

**参考答案：** 真实模型存在网络、额度、延迟和输出随机性，会让失败原因混在 provider 状态与本地代码之间。scripted model 能固定每一步输出，让测试只验证消息协议和控制流。真实连接仍有价值，但应作为显式启用的集成测试，而不是核心单元测试的前提。

### 6. 模型适配器接口在架构中承担什么责任？

**参考答案：** 它把上层统一消息和工具定义转换成 provider 请求，再把 provider 响应归一化为 `AgentStep`。adapter 不应执行工具、决定权限或持久化 session；这些属于 harness 其他层。清晰边界保证 agent loop 面向稳定抽象编程，而 provider 特有错误和字段处理集中在一处。

### 7. 场景题：`ScriptedModel` 的脚本步骤提前耗尽，测试应该如何处理这个现象？

**参考答案：** 应把它视为测试剧本与实际调用次数不一致，而不是静默重复最后一步。先检查循环是否发生了意外重试或多调用，再检查测试是否少提供了工具执行后的 final step。让脚本耗尽显式失败，可以暴露控制流回归；如果自动兜底，测试可能在错误调用次数下仍然通过。

### 8. 当前核心类型与真实 MiniCode-Python 相比为什么可以更简单？

**参考答案：** 阶段 1 只需要支持 assistant 和工具调用的最小闭环，因此不必提前加入 streaming chunk、完整诊断、session 元数据或 turn phase。保留稳定的核心语义和 adapter 抽象即可。后续只有在控制流、产品界面或持久化真正需要时再扩展字段，可以避免类型成为尚未理解的复杂度仓库。
