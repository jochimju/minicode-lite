...
### 1. 为什么路径访问、文件编辑和命令执行要设计成三类权限检查？

**参考答案：** 三者回答不同问题：路径检查判断动作发生在哪里，编辑审批判断用户是否同意具体内容变化，命令审批判断进程能力和副作用风险。目标在 cwd 内只说明位置合法，不代表覆盖文件或执行 `git reset --hard` 已获同意。拆分后策略、提示内容和测试都能针对真实风险，而不是依赖一个含义模糊的“已授权”布尔值。

### 2. 什么是 fail closed？阶段 7 在哪些地方采用了它？

**参考答案：** fail closed 指审批信息缺失、决策未知或权限系统无法确认时默认拒绝，而不是默认放行。headless 没有 prompt handler 时会拒绝编辑和非只读命令，prompt handler 返回拼错的 decision 也按拒绝处理；命令工具在没有权限对象时只允许保守只读集合。安全边界宁可产生可恢复失败，也不能把“未配置”解释成授权。

### 3. `allow_once`、`allow_turn` 和 `allow_all_turn` 的作用域分别是什么？

**参考答案：** `allow_once` 只批准当前这次检查，不写入缓存；`allow_turn` 在当前 agent turn 内复用同一目标或命令的授权；`allow_all_turn` 在当前 turn 内放行同类全部操作。turn 结束后必须清空临时状态，且异常路径也要通过 `finally` 清理，否则临时授权会泄漏到下一轮。

### 4. 为什么普通命令应尽量使用 argv 加 `shell=False`？

**参考答案：** argv 保留参数边界，操作系统直接启动目标程序，不会把 `|`、`&`、重定向和变量展开解释成额外命令。`shell=True` 会扩大输入语义并引入注入风险。只有确实需要复合 shell 语法、且完整命令已经经过风险识别和审批时，才应显式启动 shell。

### 5. 只维护一个“危险命令名称列表”为什么不够？

**参考答案：** 风险取决于完整语义而不只是第一个单词。例如 `git status` 通常只读，`git reset --hard` 会丢失修改；`echo ok` 无害，`echo ok | sh` 改变了执行链。还要识别解释器、下载后执行、删除参数和 Windows 的 `%`、`!`、`^` 展开。分类规则仍不可能证明命令绝对安全，所以未知或非只读命令也应进入审批。

### 6. 为什么文件修改要先在内存中生成 diff，再请求审批，最后写盘？

**参考答案：** 用户应批准将要发生的具体变化，而不是一个抽象的“允许编辑”。先验证所有 replacement 并生成最终内容，可以避免让用户批准一个实际不会成功或只会应用一半的补丁；审批拒绝时也保证磁盘完全不变。diff 还应有长度上限，防止大文件撑满终端或模型上下文。

### 7. 场景题：用户拒绝了危险命令，但进程仍然启动了，你会怎样定位？

**参考答案：** 先检查风险分类和 `ensure_command()` 是否发生在 `subprocess.run()` 之前，再确认权限异常是否被误吞后继续执行；随后用 monkeypatch 把进程启动函数替换成“一旦调用就让测试失败”，证明拒绝路径没有副作用。还要检查 shell snippet 是否在 token 化后丢失控制符，以及显式 args 路径是否绕过了完整命令检查。

### 8. 超时和输出截断为什么也属于命令工具的安全边界？

**参考答案：** 权限批准只解决“能不能执行”，不限制资源消耗。无超时的命令可能永久占住 agent loop，超大 stdout/stderr 会挤爆模型上下文和日志。超时应终止直接子进程并返回部分输出；截断应保留头尾，让启动背景和末尾错误都可见。生产级系统还需处理进程树、后台任务和流式背压。

### 9. 当前权限系统与真实 MiniCode-Python 相比还缺少什么？为什么阶段 7 没有一次实现完？

**参考答案：** 当前没有持久化 `allow_always/deny_always`、auto mode、审批审计、完整 TTY/TUI 界面、后台命令和平台级隔离。阶段 7 先证明“规范化、分类、审批、副作用”的顺序和 turn 生命周期，因为这是后续功能的基础。一次复制完整策略会把产品交互、持久化和进程管理混在一起，难以确认最核心安全边界是否真的有效。

### 10. 路径经过 `resolve()` 并检查 workspace 后，是否就绝对安全？

**参考答案：** 不是。它能防御常见 `..`、绝对路径和符号链接逃逸，但检查与真正打开文件之间仍可能发生 TOCTOU：目标或链接在两步之间被替换。当前学习实现建立了合理的应用层边界；更高安全要求下可使用目录文件描述符、禁止链接跟随、容器或操作系统沙箱，并对实际打开对象再次验证。
